...
import { addPatient, searchPatients, getPatientByHN } from '../firebase/firestore';

const CLAUDE_API_KEY_STORAGE = 'dental_claude_key';

export default function PatientModal({ onSelect, onClose }) {
  const [tab, setTab] = useState('search'); // search | screenshot | new
  ...
  const handleReadScreenshot = async () => {
    if (!imgBase64) return;
    if (!apiKey) { setError('กรุณากรอก Claude API Key ก่อน'); return; }
...
{
  "hn": "รหัสคนไข้ HN หรือ patient ID",
  "firstName": "ชื่อ",
  "lastName": "นามสกุล",
  "phone": "เบอร์โทรศัพท์",
  "dob": "วันเกิด ในรูปแบบ YYYY-MM-DD ถ้ามี"
}
ถ้าไม่พบข้อมูลใด ให้ใส่ค่าว่าง "" สำหรับ field นั้น ตอบเป็น JSON อย่างเดียวเท่านั้น`
              }
            ]
          }]
        })
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          {error && (
            <div style={{
              ...
              padding: '8px 12px', color: '#dc2626', fontSize: '13px', marginBottom: '12px'
            }}>{error}</div>
          )}
            ...
            <div>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                ...
                <button onClick={handleSearch} style={{
                  padding: '9px 16px', background: '#34a878', color: 'white',
            ...
            <div>
              <div style={{ marginBottom: '14px' }}>
                ...
                <p style={{ fontSize: '11px', color: '#9ca3af', margin: '4px 0 0' }}>
                  บันทึกไว้ในเครื่องนี้เท่านั้น ไม่ได้ส่งไปที่อื่น
                </p>
              </div>
              ...
              <button onClick={() => fileRef.current.click()} style={{
                ...
                fontSize: '14px', color: '#34a878', fontWeight: '600',
              ...
              {imgPreview && (
                <>
                  ...
                  <button onClick={handleReadScreenshot} disabled={aiReading} style={{
                    width: '100%', padding: '11px',
                    background: aiReading ? '#9ca3af' : '#34a878',
                    color: 'white', border: 'none', borderRadius: '10px',
                    fontSize: '14px', fontWeight: '600', cursor: aiReading ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit', marginBottom: '12px'
                  }}>
              ...
              {extractedData && (
                <div style={{ border: '1.5px solid #d1fae5', borderRadius: '12px', padding: '14px', background: '#f0faf5' }}>
                  <p style={{ fontSize: '13px', fontWeight: '700', color: '#1a2e25', margin: '0 0 10px' }}>
                    ✅ ข้อมูลที่อ่านได้ — กรุณาตรวจสอบก่อนบันทึก
                  </p>
                  {['hn', 'firstName', 'lastName', 'phone', 'dob'].map(f => (
                    <div key={f} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      <label style={{ fontSize: '12px', color: '#6b7280', width: '70px', flexShrink: 0 }}>
                        {f === 'hn' ? 'HN' : f === 'firstName' ? 'ชื่อ' : f === 'lastName' ? 'นามสกุล' : f === 'phone' ? 'เบอร์โทร' : 'วันเกิด'}
                      </label>
                      <input
                        value={extractedData[f] || ''}
                        onChange={e => setExtractedData({ ...extractedData, [f]: e.target.value })}
                        style={{ ...inputStyle, flex: 1 }}
                      />
                    </div>
                  ))}
                  <button onClick={handleSaveExtracted} disabled={saving} style={{
                    width: '100%', padding: '10px',
                    background: '#34a878', color: 'white', border: 'none',
